...
export interface AnalyticsData {
  ...
  memberAttendanceStats: { memberId: string; memberName: string; attendanceCount: number; attendanceRate: number }[]
  ...
  postEngagement: { postId: string; likes: number; comments: number; engagement: number }[]
  monthlyPosts: { month: string; count: number }[]

  // Meeting Analytics
  meetingStats: { meetingId: string; title: string; attendanceCount: number; attendanceRate: number }[]
    ...
    const calculateAnalytics = () => {
      ...
      const filteredLogs = dateRange
        ? attendanceLogs.filter(
            (log) => log.checkInTimestamp >= dateRange.start && log.checkInTimestamp <= dateRange.end,
          )
        : attendanceLogs

      const filteredMeetings = dateRange
        ? meetings.filter((meeting) => meeting.date >= dateRange.start && meeting.date <= dateRange.end)
        : meetings
      ...
      const averageLateness =
        filteredLogs.length > 0
          ? filteredLogs.reduce((sum, log) => sum + (log.lateness || 0), 0) / filteredLogs.length
          : 0
      ...
      const attendanceTrend = last30Days.map((date) => {
        const dayLogs = filteredLogs.filter((log) => log.checkInTimestamp.toDateString() === date.toDateString())
        const dayMeetings = filteredMeetings.filter((meeting) => meeting.date.toDateString() === date.toDateString())
        const possibleAttendance = members.length * dayMeetings.length
        const rate = possibleAttendance > 0 ? (dayLogs.length / possibleAttendance) * 100 : 0
      ...
      const memberAttendanceStats = members
        .map((member) => {
          ...
          const attendanceRate = memberPossibleAttendance > 0 ? (memberLogs.length / memberPossibleAttendance) * 100 : 0
      ...
      // Members by Stage
      const stageGroups = members.reduce(
        (acc, member) => {
      ...
      const membersByStage = Object.entries(stageGroups).map(([stage, count]) => ({
        stage:
          stage === "primary"
            ? "ابتدائي"
            : stage === "preparatory"
      ...
      const memberGrowth = Array.from({ length: 12 }, (_, i) => {
        ...
        const totalMembers = members.filter((member) => member.createdAt <= monthEnd).length

        const newMembers = members.filter(
          (member) => member.createdAt >= monthStart && member.createdAt <= monthEnd,
        ).length
      ...
      const monthlyPosts = Array.from({ length: 12 }, (_, i) => {
        ...
        const monthPosts = posts.filter((post) => post.createdAt >= monthStart && post.createdAt <= monthEnd).length
      ...
      const meetingStats = filteredMeetings
        .map((meeting) => {
          const meetingLogs = filteredLogs.filter((log) => log.meetingId === meeting.id)
          const attendanceRate = members.length > 0 ? (meetingLogs.length / members.length) * 100 : 0
      ...
      const weeklyAttendance = Array.from({ length: 8 }, (_, i) => {
        ...
        const weekLogs = filteredLogs.filter(
          (log) => log.checkInTimestamp >= weekStart && log.checkInTimestamp <= weekEnd,
        )
      ...
      const averageAttendance = filteredMeetings.length > 0 ? filteredLogs.length / filteredMeetings.length : 0

      const totalEngagement = posts.reduce((sum, post) => sum + post.likes.length + post.comments.length, 0)
      const engagementRate =
        posts.length > 0 && members.length > 0 ? (totalEngagement / (posts.length * members.length)) * 100 : 0
