...
export class ExcelService {
  ...
  static exportMembers(members: Member[]): void {
    const excelData: ExcelMember[] = members.map((member) => ({
      ...
      "المرحلة الدراسية": member.classStage === "university" ? "جامعي" : "ثانوي", // Explicitly cast to "جامعي" | "ثانوي"
    ...
    const fileName = `اعضاء_خدمة_الشباب_${new Date().toISOString().split("T")[0]}.xlsx`
    XLSX.writeFile(workbook, fileName)
  }

  // Export attendance to Excel
  static exportAttendance(attendanceLogs: AttendanceLog[], members: Member[], meetings: Meeting[]): void {
    const excelData: ExcelAttendance[] = attendanceLogs.map((log) => {
      ...
      return {
        ...
        "طريقة التسجيل": log.checkInMethod === "manual" ? "يدوي" : log.checkInMethod === "qr" ? "QR" : "مسح",
    ...
    const fileName = `سجل_الحضور_${new Date().toISOString().split("T")[0]}.xlsx`
    XLSX.writeFile(workbook, fileName)
  }

  // Import members from Excel
  static async importMembers(file: File): Promise<{ members: Partial<Member>[]; errors: string[] }> {
        ...
        try {
          ...
          jsonData.forEach((row, index) => {
            ...
            if (classStage === "university" && row["السنة الجامعية"]) {
              ...
              if (isNaN(universityYear) || universityYear < 1 || universityYear > 6) {
                errors.push(`الصف ${rowNumber}: السنة الجامعية يجب أن تكون بين 1 و 6`)
                return
              }
            }
            ...
            if (row["الهاتف الثانوي"] && !phoneRegex.test(row["الهاتف الثانوي"])) {
          ...
          reject(new Error("خطأ في قراءة الملف. تأكد من أن الملف بتنسيق Excel صحيح."))
        }
      }
  ...
  static downloadMembersTemplate(): void {
    ...
    for (let row = 1; row <= range.e.r; row++) {
      const cellAddress = XLSX.utils.encode_cell({ r: row, c: 4 }) // Column E (المرحلة الدراسية)
  ...
  static exportAttendanceReport(
    ...
    XLSX.utils.book_append_sheet(workbook, detailedSheet, "سجل الحضور التفصيلي")
    ...
    const memberStats = members.map((member) => {
      ...
      const totalLateness = memberLogs.reduce((sum, log) => sum + (log.lateness || 0), 0)
      const avgLateness = totalAttendance > 0 ? Math.round(totalLateness / totalAttendance) : 0

      return {
        "اسم العضو": member.fullName,
        "إجمالي الحضور": totalAttendance,
        "متوسط التأخير (دقيقة)": avgLateness,
        "المرحلة الدراسية": member.classStage === "university" ? "جامعي" : "ثانوي",
        "أب الاعتراف": member.confessorName,
      }
    })
    ...
    const meetingStats = meetings.map((meeting) => {
      ...
      const onTimeAttendees = meetingLogs.filter((log) => (log.lateness || 0) === 0).length
      ...
      return {
        ...
        "نسبة الحضور في الوقت": totalAttendees > 0 ? Math.round((onTimeAttendees / totalAttendees) * 100) + "%" : "0%",
      }
    })
