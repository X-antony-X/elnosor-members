...
const origin =
  process.env.NODE_ENV === "production"
    ? process.env.VERCEL_URL_FULL || "https://example.com"
    : "http://localhost:3000";

export async function POST(request: NextRequest) {
  try {
    const { userId, authenticationResponse, challengeId } =
      await request.json();
    ...
    if (!lookupId) {
      return NextResponse.json(
        { error: "Missing challenge identifier" },
        { status: 400 }
      );
    }
    ...
    if (storedUserId) {
      ...
      const userDoc = await adminDb
        .collection("webauthn_users")
        .doc(storedUserId)
        .get();
      if (userDoc.exists) {
        const credentials = userDoc.data()?.credentials || [];
        authenticator = credentials.find(
      ...
      for (const userDoc of usersQuery.docs) {
        const credentials = userDoc.data().credentials || [];
        const foundCred = credentials.find(
          (cred: any) => cred.credentialID === credentialID
        );
        if (foundCred) {
