  ...
  useEffect(() => {
    async function resolveRole() {
      ...
      try {
        ...
        let response = await fetch(`/api/admin/check-role?uid=${user.uid}`)
        let data = await response.json()

        // If main endpoint fails, try the simple fallback
        if (!response.ok || data.error) {
          console.warn("Main API failed, trying fallback:", data.error)
          response = await fetch(`/api/admin/check-role-simple?uid=${user.uid}`)
